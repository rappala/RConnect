 ...
 */
public class Adapter_Cart extends ParseQueryAdapter<Data_CartObject> {

    LruCache<String, Bitmap> eitemImageCacheMemory;
   // private ImageView trash_img;
    Data_CartObject currObj;

    public Adapter_Cart(Context context, QueryFactory<Data_CartObject> queryFactory) {
    ...
    public View getItemView(Data_CartObject object, View v, ViewGroup parent) {
        ViewHolder holder;
        currObj = object;
        Toast.makeText(getContext(), "getItemView called", Toast.LENGTH_SHORT);
        ...
        if((Boolean)object.get("isDraft") == true){
